...
include("../src/cuonic/PHPAuth2/Auth.php");
...
// Verify return code
if($register['code'] != 4) { echo "ERROR : STAGE 2 : "; var_dump($register); exit(); }
// Account is now registered. Fetch user id
$user = $auth->getUserData("testuser");
// Verify return data
if($user == false) { echo "ERROR : STAGE 3"; exit(); }
...
// Verify row count
if($query->rowCount() == 0) { echo "ERROR : STAGE 4 : "; exit(); }
// Fetch data
$row = $query->fetch(PDO::FETCH_ASSOC);
// Activate the user
$activate = $auth->activate($row['activekey']);
// Verify return code
if($activate['code'] != 5) { echo "ERROR : STAGE 5 : "; var_dump($activate); exit(); }
// Account is now activated. Attempt login
$login = $auth->login("testuser", $passwordhash);
// Verify return code
if($login['code'] != 4) { echo "ERROR : STAGE 6 : "; var_dump($login); exit(); }
// Account now logged in. Check if session hash corresponds with username
$getusername = $auth->getUsername($login['session_hash']);
// Verify returned data
if(!$getusername) { echo "ERROR : STAGE 7"; exit(); }
if($getusername != "testuser") { echo "ERROR : STAGE 8 : " . $getusername; exit(); }
// Session is linked to correct username. Check if session is valid
if(!$auth->checkSession($login['session_hash'])) { echo "ERROR : STAGE 9"; exit(); }
...
if($user['uid'] != $sessionuid) { echo "ERROR : STAGE 10 : " . $sessionuid; exit(); }
...
// Verify return code
if($changepassword['code'] != 5) { echo "ERROR : STAGE 11 : "; var_dump($changepassword); exit(); }
// Account password changed. Fetch user email
$getemail = $auth->getEmail($user['uid']);
// Verify returned data
if(!$getemail) { echo "ERROR : STAGE 12"; exit(); }
if($getemail != "testuser@test.com") { echo "ERROR : STAGE 13 : " . $getemail; exit(); }
...
// Verify return code
if($changeemail['code'] != 5) { echo "ERROR : STAGE 14 : "; var_dump($changeemail); exit(); }
// Email has been changed. Verify email change.
$getemail = $auth->getEmail($user['uid']);
// Verify returned data
if(!$getemail) { echo "ERROR : STAGE 15"; exit(); }
if($getemail != "testuser@example.com") { echo "ERROR : STAGE 16 : " . $getemail; exit(); }
// Email change confirmed. Log account out
if(!$auth->logout($login['session_hash'])) { echo "ERROR : STAGE 17"; exit(); }
